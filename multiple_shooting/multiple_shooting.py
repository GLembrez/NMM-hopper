...
dxs = (
    cs.vertcat(xs[2:], K * (1 - l) * xs[0] / l, K * (1 - l) * xs[1] / l - 1)
)
...
s2f = cs.vertcat(xs[:2], cs.atan(-xs[0] / xs[1]), xs[2:], (xs[0] * xs[3] - xs[1] * xs[2])/l)
...
def init_trajectory(x0):
    ...
    x03 = np.array([y2[0],y2[1],np.arctan(-y2[0]/y2[1]),y2[2],y2[3],y2[0]*y2[3]-y2[1]*y2[2]])
