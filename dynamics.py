...
dxs = cs.vertcat(
    xs[2:], params.K * (1 - l) * xs[0] / l, params.K * (1 - l) * xs[1] / l - 1
) + xi * cs.gradient(Es, xs)
dxf = cs.vertcat(xf[3:], 0, -1, -(params.W**2) * xf[2]) + xi * cs.gradient(Ef, xf)
...
traj_f_list = RK4f.mapaccum(params.N_F)
traj_s_list = RK4s.mapaccum(params.N_S)
